...
dispatch({ type: 'UPDATE_AVALUE', payload: true });
    ...
    const gethistory=async (e)=>{
        const response=await fetch("http://35.175.147.9:5000/api/g/gatetoken",{
            method:'get',
            headers:{
...
const sendrq=async(props)=>{
  ...
  if(whatid==0){
    const response=await fetch("http://35.175.147.9:5000/api/ud/updateintime",{
      method:'POST',
      headers:{
          'Content-Type':'application/json',
  ...
  const response=await fetch("http://35.175.147.9:5000/api/ud/updateouttime",{
          method:'POST',
          headers:{
              'Content-Type':'application/json',
              'auth-token':localStorage.getItem('admintoken')
