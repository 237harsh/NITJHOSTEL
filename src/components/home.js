...
export const Home = () => {
 ...
 const getuserdata=async()=>{
  const response=await fetch("http://35.175.147.9:5000/api/auth/getuser",{
    method:'get',
    headers:{
...
dispatch({ type: 'UPDATE_photo_url', payload: `http://35.175.147.9:5000/api/a/newupload/${json.user.photo_url}` });
NODisableli()
}else{
 ...
 const getroomnumbers=async()=>{
  const response=await fetch("http://35.175.147.9:5000/api/b/roomnumbers",{
    method:'get',
    headers:{
        'Content-Type':'application/json',
        'auth-token':localStorage.getItem('token')
...
const rangto=(e)=>{
  ...
  const handle=async (e)=>{
    ...
    e.preventDefault();
    const response=await fetch("http://35.175.147.9:5000/api/b/bookroom",{
        method:'POST',
        headers:{
            'Content-Type':'application/json',
            'auth-token':localStorage.getItem('token')
